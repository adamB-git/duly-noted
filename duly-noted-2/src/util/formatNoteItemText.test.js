...
function getRandomStringOfLength(ln) {
 const randomChars = Array(ln).fill(null).map(() => Math.floor((Math.random()
* 25) + 65));
...
() => {
 ...
 const formattedText = formatNoteItemText(text);
 expect(formattedText).toBe(`${text.substring(0, 200)}...`);
});
...
`;
 ...
 const formattedText = formatNoteItemText(text);
 expect(formattedText).toBe(`Satisfied conveying an dependent contented he
